...
def run_exp(exp_id, res_dir, K, T, lead_time, seq_length,            
            ...
            da_N, da_Inf, da_Bx, da_rot, da_loc_rad, da_xN, da_Lag, **net_kwargs):
    ...
    # data = np.zeros(shape=(tseq.Ko+1, N_trials, tseq.dko+3, Nx), dtype=np.float32)
...
#     summary(da_model, (obs_num+1, K))
    ...
    for k in range(10):
        ...
        if da_method == 'Var4D':
            xp = da.Var4D(B='clim', Lag=1, nIter=5, xB=da_Bx, wtol=1e-4)
        if da_method == 'DEnKF':
    ...
    logger.info(f'DeepDANet analysis rmse decrease (mean): {np.mean((np.array(err_xa_c)-np.array(err_xa_n))/np.array(err_xa_c))}, DeepDANet analysis rmse decrease (std): {np.std((np.array(err_xa_c)-np.array(err_xa_n))/np.array(err_xa_c))}')
    logger.info(f'DeepDANet forecast rmse decrease (mean): {np.mean((np.array(err_xb_c)-np.array(err_xb_n))/np.array(err_xa_c))}, DeepDANet forecast rmse decrease (std): {np.std((np.array(err_xb_c)-np.array(err_xb_n))/np.array(err_xa_c))}')
    logger.info(f'DeepDANet da time decrease (mean): {np.mean((np.array(time_c)-np.array(time_n))/np.array(time_c))}, DeepDANet da time decrease (std): {np.std((np.array(time_c)-np.array(time_n))/np.array(time_c))}')
